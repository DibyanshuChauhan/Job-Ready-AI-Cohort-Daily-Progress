...
const Sidebar = ({
  ...
  // Filter the chats based on the active sidebar tab selection
    const chatList = Object.values(chats);
    const filteredChats = chatList.filter((chatItem) => {
    // If a chat contains a message with role === 'email', we tag it as an email thread
    const hasEmailActions = chatItem.messages?.some(
    (msg) => msg.role === "email",
    );
    return activeTab === "emails" ? hasEmailActions : !hasEmailActions;
});
